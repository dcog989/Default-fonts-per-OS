...
export function renderCompareView(app) {
  ...
  const allFonts = app.state.fontData.operatingSystems.flatMap((os) => os.fonts);
  const fontsToCompare = [...new Map(allFonts.map((f) => [f.name, f])).values()].filter((font) =>
    app.state.comparisonSet.has(font.name),
  );
