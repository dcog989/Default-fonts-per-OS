...
export function persistOrder(app) {
  const domOrder = [...app.elements.content.querySelectorAll('.os-set')].map((el) => el.dataset.osName);

  if (!app.state.fontData) return;
  app.state.fontData.operatingSystems = reorderOperatingSystems(app.state.fontData.operatingSystems, domOrder);
